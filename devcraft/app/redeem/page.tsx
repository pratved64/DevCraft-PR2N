...
import { ShoppingBag, X, Battery, Wifi, Search, ArrowRightLeft, QrCode, MapPin } from 'lucide-react';
...
// Mapped sponsor data for map view
const MOCK_SPONSORS = [
  { _id: "sp_1", name: "TechCorp", loc: { x: 120, y: 450 } },
  { _id: "sp_2", name: "DevStudio", loc: { x: 300, y: 200 } }
];
...
export default function PokemonDash() {
  ...
  // Boot Effect
  useEffect(() => { setTimeout(() => setBooting(false), 1500); }, []);
  ...
  const handleLegendaryTrade = (scanToSacrifice: any) => {
    // Remove from Pokedex
  ...
  const generateVoucher = (item: any, type: string) => {
    setGeneratedVoucher({
      title: item.item_name,
      code: `${type.substring(0,3)}-${Math.floor(Math.random() * 99999)}`,
      desc: type
    });
  };
                      ...
                      {view === 'MAP' && (
                        <div className="h-full relative bg-[#e0f8cf] border border-[#0f380f]">
                           ...
                           {MOCK_SPONSORS.map(sp => (
                             <div key={sp._id} className="absolute flex flex-col items-center" style={{ left: sp.loc.x / 2, top: sp.loc.y / 2 }}>
