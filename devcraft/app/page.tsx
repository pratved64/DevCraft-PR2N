  ...
  <div className="fixed top-6 left-0 w-full z-50 flex justify-center px-4 animate-fade-in">
    <nav className="glass-panel rounded-full px-6 py-3 flex items-center gap-8 max-w-3xl w-full justify-between">
      ...
      <div className="hidden md:flex items-center gap-6">
        {['Product', 'Solutions', 'Developers'].map((link) => (
          <a key={link} href="#" className="text-sm font-medium text-gray-400 hover:text-white transition-colors hover:shadow-glow">
            {link}
          </a>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        <button className="text-sm font-medium text-white hover:text-cyan-400 transition-colors px-3">
          Log in
        </button>
        <button className="px-5 py-2 rounded-full bg-white text-black text-sm font-bold hover:bg-cyan-50 transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.3)]">
          Get Access
        </button>
      </div>
    </nav>
...
const Hero = () => (
  <section className="relative min-h-screen flex flex-col justify-center items-center text-center px-4 pt-20">
    ...
    <div className="absolute bottom-12 left-12 hidden lg:flex gap-4">
      <MetricCard label="Active Sensors" value="842" trend="+12%" delay="delay-300" />
      <MetricCard label="Data Velocity" value="2.4 GB/s" delay="delay-300" />
    </div>
  </section>
);
