...
const MOCK_USER = {
    ...
    wallet: { total_points: 0, legendaries_caught: 0 }
};
...
export default function PokemonDash() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                ...
                const userData = await fetchMyHistory();
                setUser(prev => ({
                    ...prev,
                    _id: userData.user_id,
                    name: userData.name,
                    wallet: { total_points: userData.total_points, legendaries_caught: userData.legendaries_caught },
                }));
                setApiUserId(userData.user_id);

                if (userData.pokedex && userData.pokedex.length > 0) {
                    setScans(userData.pokedex.map((p: any, i: number) => {
                ...
                // 3. Fetch Stalls for Map
                const stallsData = await fetchStalls();
                setStalls(stallsData);

            } catch (error) {
                console.error("Failed to load redeem data", error);
            }
        };
        ...
        if (user.wallet.total_points >= item.cost_in_points) {
            try {
                ...
                if (result.success) {
                    ...
                    const newRewards = await fetchRewards();
                    setRewards(newRewards.map(r => ({
                        _id: r.id,
                        item_name: r.item_name,
                        category: r.category,
                        cost_in_points: r.cost_in_points,
    ...
    const handleLegendaryTrade = async (scanToSacrifice: any) => {
        try {
            ...
            if (result.success) {
                ...
                setUser(prev => ({
                    ...
                    wallet: {
                        ...
                        legendaries_caught: Math.max(0, prev.wallet.legendaries_caught - 1),
                    },
                }));
            ...
            alert("Trade failed. Check connection.");
        }
    };
                                    ...
                                    <div className="flex-1 overflow-y-auto pixel-scrollbar p-2">
                                        ...
                                        {view === 'MAP' && (
                                            <div className="h-full relative bg-[#e0f8cf] border border-[#0f380f]">
                                                ...
                                                {stalls.length === 0 ? (
                                                    <div className="absolute inset-0 flex items-center justify-center font-pixel text-[8px]">LOADING MAP DATA...</div>
                                                ) : (
                                                    stalls.map(sp => (
                                                        <div
                                                            key={sp.stall_id}
                                                            ...
                                                            style={{
                                                                left: `${Math.min(90, Math.max(10, (sp.map_location.x_coord / 900) * 100))}%`,
                                                                top: `${Math.min(90, Math.max(10, (sp.map_location.y_coord / 900) * 100))}%`,
                                                                transform: 'translate(-50%,-50%)'
                                                            }}
                                                        >
                                                            <MapPin size={16} className="text-[#0f380f] animate-bounce" />
                                                            <span className="font-pixel text-[6px] bg-[#0f380f] text-[#9bbc0f] px-1 whitespace-nowrap">{sp.company_name}</span>
                                                        </div>
                                                    ))
                                                )}
                                            </div>
